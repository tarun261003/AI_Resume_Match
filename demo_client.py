...
from config import OPEN_API_KEY,QDRANT_API_KEY
...
qdrant_client = QdrantClient(
    url="https://102c37c2-e097-4063-871e-2021fb867dc6.us-east4-0.gcp.cloud.qdrant.io:6333", 
        ...
        if st.button("Check Match"):
            with st.spinner('Checking resume'):
                try:
                    ...
                    refresh()
                    st.markdown(f'<div id="similarity-score" style="color:#D80032;">Issue with API Response please refreshing the page.</div>', unsafe_allow_html=True)
    else:
        if resume_file is not None and jd_file is None:
            st.write("Please upload a Job description in Supported format.")
        elif resume_file is None and jd_file is not None:
            st.write("Please upload a Resume in Supported format.")
        else:
            st.write("Please upload both a resume and a job description in Supported format.")
        ...
        if st.button("Check Match"):
            with st.spinner('Checking resume'):
                ...
                try:
                    ...
                    refresh()
                    # st.toast('Refresh',icon='🔃')
                    # st.markdown(f'<div id="similarity-score" style="font-size:20px; margin-left:20px;">Issue with API Response Please Refresh</div>', unsafe_allow_html=True)
